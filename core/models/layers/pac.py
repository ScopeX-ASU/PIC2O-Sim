...
# from torch._thnn import type2backend # not supported in the latest pytorch
from mmengine.registry import MODELS
from functools import lru_cache
from einops import einsum
try:
    import pyinn.pyinn as P
...
class GaussKernel2dFn(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_diff = (feat_0 - cols).mul_((grad_output * output).expand_as(cols))


        grad_diff[:, :, center_y:center_y + 1, center_x:center_x + 1, :, :] -= \
            grad_diff.sum(dim=(2,3), keepdim=True)
...
class PacConv2dFn(Function):
    ...
    def forward(ctx, input, kernel, weight, bias=None, stride=1, padding=0, dilation=1, shared_filters=False):
        ...
        if shared_filters:
            ...
            output = einsum(in_mul_k, weight, 'i j k l m n, o j k l -> i o m n')
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
            if ctx.shared_filters:
                ...
                grad_in_mul_k = einsum(grad_output, weight, 'i o m n, o j k l -> i j k l m n')
        ...
        if ctx.needs_input_grad[1]:
            ...
            grad_kernel = grad_in_mul_k.mul_(in_cols)
        ...
        if ctx.needs_input_grad[2]:
            ...
            if ctx.shared_filters:
                ...
                grad_weight = einsum(grad_output, in_mul_k, 'i o m n, i j k l m n -> o j k l')
...
class PacConvTranspose2dFn(Function):
    ...
    def forward(ctx, input, kernel, weight, bias=None, stride=1, padding=0, output_padding=0, dilation=1,
        ...
        ctx.save_for_backward(input if (ctx.needs_input_grad[1] or ctx.needs_input_grad[2]) else None,
                              kernel if (ctx.needs_input_grad[0] or ctx.needs_input_grad[2]) else None,
                              weight if (ctx.needs_input_grad[0] or ctx.needs_input_grad[1]) else None)
        ctx._backend = "cuda" if input.is_cuda else "cpu"
        # ctx._backend = torch.backends.cuda.get_backend_name() if input.is_cuda else "cpu"
...
def pacconv2d(input, kernel, weight, bias=None, stride=1, padding=0, dilation=1, shared_filters=False,
              native_impl=False, parallel_weights=False):
    if parallel_weights:
        sub_weight_1, sub_weight_2, sub_weight_3 = weight
        sub_weight_1_kernel_size = sub_weight_1.shape[-2:]
        sub_weight_2_kernel_size = sub_weight_2.shape[-2:]
        sub_weight_3_kernel_size = sub_weight_3.shape[-2:]
        index_gap_3_1 = (sub_weight_3_kernel_size[0] - sub_weight_1_kernel_size[0]) // 2
        index_gap_3_2 = (sub_weight_3_kernel_size[0] - sub_weight_2_kernel_size[0]) // 2
        # weight = torch.zeros_like(sub_weight_3)
        # weight[:, :, :, :] += sub_weight_3
        weight = sub_weight_3.clone()
        weight[:, :, index_gap_3_2: -index_gap_3_2, index_gap_3_2: -index_gap_3_2] += sub_weight_2
        weight[:, :, index_gap_3_1: -index_gap_3_1, index_gap_3_1: -index_gap_3_1] += sub_weight_1
    ...
    if native_impl:
        ...
        if shared_filters:
            ...
            output = einsum(im_cols * kernel, weight, 'i j k l m n, o j k l -> i o m n')
...
class _PacConvNd(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride,
                 ...
                 channel_wise, normalize_kernel, shared_filters, filler, parallel_weights):
        ...
        if not pool_only:
            ...
            elif parallel_weights:
                exit('parallel_weights should not be triggered anymore')
                assert len(kernel_size) == 2, 'parallel_weights only supports 2D kernels'
                if kernel_size[0] <= 9: # if kernel size is small, use 3 as the easy-to-optimize small kernel
                    small_kernel_size = 3
                else:   # otherwise, use 5 as the small kernel
                    small_kernel_size = 5
                sub_weight_1_kernel_size = (small_kernel_size, small_kernel_size) # treat 3 or 5 as a common small kernel that is easy to optimize
                sub_weight_2_kernel_size = (1 + (small_kernel_size + kernel_size[0] - 2) // 2, 1 + (small_kernel_size + kernel_size[0] - 2) // 2)
                if sub_weight_2_kernel_size[0] % 2 == 0: # must be odd number
                    sub_weight_2_kernel_size = (sub_weight_2_kernel_size[0] - 1, sub_weight_2_kernel_size[1] - 1)
                sub_weight_3_kernel_size = kernel_size
                self.sub_weight_1 = Parameter(torch.Tensor(in_channels, out_channels, *sub_weight_1_kernel_size))
                self.sub_weight_2 = Parameter(torch.Tensor(in_channels, out_channels, *sub_weight_2_kernel_size))
                self.sub_weight_3 = Parameter(torch.Tensor(in_channels, out_channels, *sub_weight_3_kernel_size))
                # Initialize weights
                torch.nn.init.kaiming_uniform_(self.sub_weight_1, a=math.sqrt(5))
                torch.nn.init.kaiming_uniform_(self.sub_weight_2, a=math.sqrt(5))
                torch.nn.init.kaiming_uniform_(self.sub_weight_3, a=math.sqrt(5))
            else:
    ...
    def reset_parameters(self):
        if not (self.pool_only or self.filler in {'pool', 'crf_pool'}):
            if self.filler == 'uniform':
                ...
                if not self.parallel_weights:
                    self.weight.data.uniform_(-stdv, stdv)
                else:
                    pass
                    # the weights are initialized in the __init__ function
    ...
    def extra_repr(self):
        ...
        if self.parallel_weights:
            s += ', parallel_weights=True'
...
class PacConv2d(_PacConvNd):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True,
                 ...
                 filler='uniform', native_impl=True, device='cuda'):
        self.parallel_weights = False
        kernel_size = _pair(kernel_size)
        stride = _pair(stride)
        padding = _pair(padding)
        dilation = _pair(dilation)
        super(PacConv2d, self).__init__(
            ...
            False, kernel_type, smooth_kernel_type, False, normalize_kernel, shared_filters, filler, self.parallel_weights)
    ...
    def forward(self, input_2d, input_for_kernel, kernel=None, mask=None):
        ...
        if self.parallel_weights:
            output = pacconv2d(input_2d, kernel, (self.sub_weight_1, self.sub_weight_2, self.sub_weight_3), self.bias, self.stride, self.padding, self.dilation,
                            self.shared_filters, self.native_impl, self.parallel_weights)
        else:
